...
class SceneSegModel(object):
    ...
    def get_weighted_loss(self, num_per_class_weights):
        class_weights = tf.convert_to_tensor(num_per_class_weights, dtype=tf.float32)

        one_hot_labels = tf.one_hot(self.inputs['point_labels'], depth=self.config.num_classes)
        weights = tf.reduce_sum(class_weights * one_hot_labels, axis=1)
        unweighted_losses = tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.logits,
                                                                       labels=one_hot_labels,
                                                                       name='cross_entropy')
        weighted_losses = unweighted_losses * weights
        cross_entropy = tf.reduce_mean(weighted_losses, name='cross_entropy_mean')
