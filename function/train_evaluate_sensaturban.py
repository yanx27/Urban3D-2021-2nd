"""
Training and evaluating script for scene segmentation with SensatUrban dataset
"""
import os
import sys
import time
...
from utils.scheduler import StepScheduler, CosineDecayScheduler
from utils.metrics import AverageMeter, SensatUrban_metrics, SensatUrban_subset_metrics, SensatUrban_voting_metrics


def parse_option():
    parser = argparse.ArgumentParser("Training and evaluating SensatUrban")
    ...
    parser.add_argument('--trainval', action='store_true', default=False, help='use validation set in trianing')
...
def training(config):
    with tf.Graph().as_default():
        ...
        # TODO CosineDecayScheduler
        # lr_scheduler = CosineDecayScheduler('learning_rate',
        #                              config.base_learning_rate * config.batch_size * config.num_gpus / 8.0,
        #                              config.max_epoch)
        ...
        for i, igpu in enumerate(config.gpus):
            with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                with tf.device('/gpu:%d' % (igpu)), tf.name_scope('gpu_%d' % (igpu)) as scope:
                    ...
                    # model.get_loss()
                    # TODO weighted loss
                    model.get_weighted_loss(dataset.num_per_class_weights)
        ...
        for i, label_value in enumerate(dataset.label_values):
            val_proportions[i] = np.sum([np.sum(labels == label_value) for labels in dataset.val_labels])
...
def val_one_epoch(sess, ops, dataset, validation_probs, val_proportions, epoch):
    ...
    IoUs, mIoU = SensatUrban_subset_metrics(dataset, predictions, targets, val_proportions)
    vote_IoUs, vote_mIoU = SensatUrban_voting_metrics(dataset, validation_probs, val_proportions)
    ...
    while last_min < num_votes:
        try:
            ...
            if last_min + 1 < new_min:
                ...
                IoUs, mIoU = SensatUrban_voting_metrics(dataset, val_probs, val_proportions)
                ...
                if int(np.ceil(new_min)) % 2 == 0:
                    ...
                    for i, file_path in enumerate(files):
                        ...
                        if cloud_name in dataset.val_file_name:
                            # Reproject probs on the evaluations points
                            probs = val_probs[i_val][dataset.val_proj[i_val], :]
                            proj_probs += [probs]
                            i_val += 1
                    # Show vote results
                    logger.info('==> Confusion on full clouds')
                    IoUs, mIoU = SensatUrban_metrics(dataset, proj_probs)
